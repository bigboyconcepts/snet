...
public class Request implements Serializable
{
    ...
    public Request setPostMethod(@Nonnull PostMethod postMethod)
    {
        ...
        {
            if(Http.LOGGING) Log.w(Http.LOG_TAG, "Warning. requestBody is not empty, it will be ignored since new PostMethod is not PostMethod.BODY");
        }
        if(postMethod != PostMethod.FORM_DATA && (files != null && files.length > 0))
        {
            if(Http.LOGGING)Log.w(Http.LOG_TAG, "Warning. Files will be ignored since new PostMethod is not PostMethod.FORM_DATA");
        }
        return this;
    }
    ...
    public Request addParam(String key, String value, boolean forceAddToUrl)
    {
        ...
        {
            if(Http.LOGGING)Log.e(Http.LOG_TAG, "RequestBuilder >> addParam : param not set");
            return this;
        }
        if(forceAddToUrl || mMethod == Method.GET || mMethod == Method.DELETE)
        {
            urlParams.put(key, HttpUtility.encodeString(value));
        }
    ...
    public Request setResponseMessagePolicy(ResponseHandler.ResponseMessagePolicy responseMessagePolicy)
    {
