...
public abstract class RequestManager
{
    ...
    private RequestHandler mGlobalRequestHandler;
    ...
    private void assertRequestHandler(RequestHandler requestHandler)
    {
    ...
    private void handlePostRequest(final int requestCode, final Request request, final ResponseParser responseParser, boolean sync)
    {
        ...
        runOnUIThread(new Runnable()
        {
            ...
            public void run()
            {
                new ResponseHandlerImpl(requestCode, responseParser, request.getResponseMessagePolicy())
                {
                    @Override
                    public void onResponse(int requestCode, int responseStatus, ResponseParser responseParser)
                    {
                        notifyResponseHandler(requestCode, responseStatus, responseParser);
                    }
                };
            }
        });
